...
import { useDeviceInfo, useDeviceData, useDeviceHistory, useLatestValues, useTimeContext, useMappings } from '../../hooks';
import { Sparkline } from './Sparkline';

export function DeviceDrawer({ onClose }) {
  const name = useSelectedDevice();
  const info = useDeviceInfo(name);
  const latestLocal = useDeviceData(name);
  const history = useDeviceHistory(name);
  const latestExternal = useLatestValues(20000);
  ...
  return (
    <aside className="device-drawer" aria-labelledby="drawer-title">
      ...
      <footer>
        <button onClick={()=> { /* TODO: toggle pin via PATCH */ }}>Pin/Unpin</button>
